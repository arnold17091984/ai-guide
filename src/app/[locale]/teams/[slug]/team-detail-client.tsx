  ...
  return (
    <>
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className="relative -mx-4 -mt-8 mb-10 overflow-hidden rounded-b-3xl bg-linear-to-br from-cyan-600 via-blue-500 to-indigo-500 px-8 py-14 sm:-mx-6 lg:-mx-8"
      >
        ...
        <div className="relative">
          <motion.div variants={fadeUp} className="flex items-center gap-5">
            ...
            <div>
              ...
              {team.description && (
                <p className="mt-1 max-w-xl text-white/80">{team.description}</p>
              )}
            </div>
          </motion.div>
          ...
          <motion.div variants={fadeUp} className="mt-6 flex flex-wrap gap-3">
            ...
            {!isMember && team.isPublic && currentUserId && (
              <button
                onClick={handleJoin}
                disabled={isPending}
                className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-medium text-cyan-700 transition-colors hover:bg-white/90 disabled:opacity-50"
              >
                {labels.joinTeam}
              </button>
            )}
            ...
            {isMember && !isOwner && (
              <button
                onClick={handleLeave}
                disabled={isPending}
                className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2 text-sm font-medium text-white/80 backdrop-blur-sm transition-colors hover:bg-red-500/30 hover:text-white"
              >
                {labels.leaveTeam}
              </button>
            )}
      ...
      {/* Members */}
      <ScrollFadeIn>
        <div className="rounded-2xl border border-(--border) bg-white/70 p-6 shadow-md backdrop-blur-xl dark:bg-white/5">
